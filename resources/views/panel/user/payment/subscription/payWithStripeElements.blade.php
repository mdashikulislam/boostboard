            ...
            <div class="row row-cards">
              <div class="col-sm-8 col-lg-8">
                @include('panel.user.payment.coupon.index')
                  ...
                  <form id="payment-form" action="{{ route('dashboard.user.payment.stripeSubscribePay') }}"
                      method="post">
                      @csrf
                  ...
                  <p></p>
                  <p>{{ __('By purchase you confirm our') }} <a
                  ...
                  <div
                      ...
                      <div class="card-body flex flex-col !p-[45px_50px_50px] text-center">
                          <div class="text-heading flex items-end justify-center mt-0 mb-[15px] w-full text-[50px] leading-none">
                              ...
                              <small class="inline-flex mb-[0.3em] font-normal text-[0.35em]">/
                                  {{ $plan->frequency }}</small>
                          </div>
                          <div
                              class="inline-flex mx-auto p-[0.85em_1.2em] bg-white rounded-full font-medium text-[15px] leading-none text-[#2D3136]">
                              {{ $plan->name }}</div>
            ...
            async function handleSubmit(e) {
                ...
                if (secret.startsWith("set") == false) {
                    ...
                    error = await stripe.confirmPayment({
                        elements,
                        confirmParams: {
                            return_url: "{{ route('dashboard.user.payment.stripeSubscribePay') }}",
